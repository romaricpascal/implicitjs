...
const macro = fileBasedTest(
  __filename,
  (t, { inputPath, data, options = {} }) => {
    const template = compileFile(inputPath, options);
...
function fileBasedTest(testFileName, fn) {
  const macro = withFixtures(
    testFileName,
    withInputPath(withModules(['options', 'data'], withOutputComparison(fn)))
  );
